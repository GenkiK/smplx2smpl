...
def interpolate_points(p0: np.ndarray, p1: np.ndarray, n_nan_frames: int):
    """
    2つの3次元点 p0, p1 の間を等間隔で線形補間する。

    Parameters:
        p0 (array-like): 始点（長さ3）
        p1 (array-like): 終点（長さ3）
        n_nan_frames (int): 補間点の数（両端を含まない）

    Returns:
        np.ndarray: 補間された n_frames x 3 の配列
    """
    n_frames = n_nan_frames + 2  # 両端の点を含む
    p0 = np.asarray(p0)
    p1 = np.asarray(p1)
...
def interp_nan(src_dir: Path, tgt_dir: Path) -> None:
    ...
    for frame_idx, smpl_path in tqdm(enumerate(sorted(src_dir.glob("*.npz")))):
        ...
        if np.isnan(smpl_arr).any():  # nan
        ...
        else:  # not nan, is_nan_seq; nanが終わったのでinterpolate
            is_nan_seq = False
            next_result = smpl_arr
            print(nan_frame_idxs, flush=True)

            if prev_result is None:
                # frame_idx=0からnanが始まっている場合
                for i, nan_frame_idx in enumerate(nan_frame_idxs):
                    if i == 0:
                        assert nan_frame_idx == 0, "nanが始まる前のフレームはnanではないはず"
                ...
                # pose_paramsをslerpで補間
                prev_pose = prev_result[6:75].reshape(-1, 3)
                next_pose = next_result[6:75].reshape(-1, 3)
                interp_pose = np.empty((num_nans, 23, 3), dtype=np.float32)  # 69はSMPLのpose_paramsの数
            ...
            np.savez(tgt_dir / f"{frame_idx:010d}.npz", arr=smpl_arr.astype(np.float32), gender=gender)  # 保存
            nan_frame_idxs = []  # reset nan_frame_idxs
            prev_result = smpl_arr

    if is_nan_seq:  # 最後のnanが終わっていない場合
        # 全てprev_resultで補間
        for i, nan_frame_idx in enumerate(nan_frame_idxs):
